...
- Le calcul des `Poids` est décrit précisément dans l'ADR-022. (philosophie cf [[Poids]]). 
- Les dossiers structurants sont décrits dans [[Structure des Dossiers]]
- Les différentes variables disponibles et calculables sont spécifiées dans [[Variables Système]]
...
Liste des variables disponibles par défaut dans [[]]
...
*   **$S$ (Structure)** : Centralité du nœud.
    *   Calcul : $S = (\alpha \cdot \log(1 + \text{In})) + (\delta \cdot \log(1 + \text{Out}))$ (in)
...
Les métadonnées des Nœuds sont mis à jour en début de session. Le poids des nouveaux Nœuds crée en cours de session est également calculé.

#### 2. Mécanismes d'État du Système (La Dynamique)
Les coefficients $\alpha, \beta, \gamma, \delta$ sont modulés par un "Méta-Contrôleur" (style épistémologique) selon le mode (/ ex Exploration, Consolidation, Approfondissement, Synthèse, ...). La **Tension d'Activation** n'est pas enregistré en temps réel (mémoire en RAM)
